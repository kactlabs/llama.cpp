...
fn test_compute_kernels_directly() {
    ...
    // Test matmul kernel
    {
        // Simple 2x2 @ 2x2 -> 2x2 (GGML convention: column-major)
        // A = [1, 2]  stored as [1, 3, 2, 4] (column-major: col0=[1,3], col1=[2,4])
        //     [3, 4]
        // B = [5, 6]  stored as [5, 7, 6, 8] (column-major: col0=[5,7], col1=[6,8])
        //     [7, 8]
        // C = A @ B:
        //   C[0,0] = 1*5 + 3*6 = 23
        //   C[1,0] = 1*7 + 3*8 = 31
        //   C[0,1] = 2*5 + 4*6 = 34
        //   C[1,1] = 2*7 + 4*8 = 46
        // Result stored column-major: [23, 31, 34, 46]
        
        let a = vec![1.0, 3.0, 2.0, 4.0]; // [2, 2] column-major
        let b = vec![5.0, 7.0, 6.0, 8.0]; // [2, 2] column-major
...
fn test_full_computation_pipeline() {
    ...
    // Compute: sum = a + b
    {
        let a_ptr = a.data().unwrap();
        let b_ptr = b.data().unwrap();
        let sum_ptr = sum.data().unwrap();
        ...
        unsafe {
            ...
            let sum_data = std::slice::from_raw_parts_mut(sum_ptr.as_ptr() as *mut f32, sum.n_elements());
    ...
    {
        let sum_ptr = sum.data().unwrap();
        let c_ptr = c.data().unwrap();
        let result_ptr = result.data().unwrap();
        
        unsafe {
            let sum_data = std::slice::from_raw_parts(sum_ptr.as_ptr() as *const f32, sum.n_elements());
            let c_data = std::slice::from_raw_parts(c_ptr.as_ptr() as *const f32, c.n_elements());
